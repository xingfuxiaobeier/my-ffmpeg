...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        av_log(NULL, AV_LOG_INFO, "video thread, get video frame pts : %lld, is key_frame : %d \n", frame->pts, frame->key_frame);
        if (ret < 0)
            goto the_end;
        if (!ret)
            continue;
...
//      of the seek_pos/seek_rel variables
        ...
        {
            av_log(NULL, AV_LOG_INFO, "dhb test, av_read_frame pkt dts : %lld, keyframe : %d, ret : %d \n", pkt->dts, (pkt->flags & AV_PKT_FLAG_KEY) , ret);
        }

        if (ret < 0) {
            av_log(NULL, AV_LOG_INFO, "dhb test, av_read_frame ret < 0, ret : %d \n", ret);
            if ((ret == AVERROR_EOF || avio_feof(ic->pb)) && !is->eof) {
                av_log(NULL, AV_LOG_INFO, "dhb test, av_read_frame ret == AVERROR_EOF, ret : %d \n", ret);
                if (is->video_stream >= 0)
                    packet_queue_put_nullpacket(&is->videoq, is->video_stream);
                if (is->audio_stream >= 0)
